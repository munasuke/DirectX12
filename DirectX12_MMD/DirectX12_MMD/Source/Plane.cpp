...
Plane::Plane(ID3D12Device* dev, const XMFLOAT3& pos, const float width, const float depth) {
	...
	std::array<PrimitiveVertex, 4> vertex {{
		{XMFLOAT3(pos.x - width / 2.0f, 0.0f, pos.z - depth / 2.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), XMFLOAT2(0, 0)},
		{XMFLOAT3(pos.x - width / 2.0f, 0.0f, pos.z + depth / 2.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), XMFLOAT2(0, 1)},
		{XMFLOAT3(pos.x - width / 2.0f, 0.0f, pos.z - depth / 2.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), XMFLOAT2(1, 0)},
		{XMFLOAT3(pos.x - width / 2.0f, 0.0f, pos.z + depth / 2.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), XMFLOAT2(1, 1)},
	}};

	auto result = dev->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE::D3D12_HEAP_TYPE_UPLOAD),
		D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(sizeof(PrimitiveVertex) * vertex.size()),
	...
	memcpy(data, &vertex, sizeof(PrimitiveVertex) * vertex.size());
	...
	vbView.SizeInBytes		= sizeof(PrimitiveVertex) * vertex.size();	//データ全体のサイズを指定
}
